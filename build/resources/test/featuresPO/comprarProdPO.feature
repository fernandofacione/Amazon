#language: pt
Funcionalidade: Comprar Produto PO
  Cenario: Pesquisar Produto com Clique e Incluir no Carrinho PO
    Dado que acesso o site da Amazon  PO
    Quando pesquiso por "Cafeteira Expresso Prima Latte II, Vermelho, 110v, Oster" PO
    E clico na Lupa PO
    Entao vejo a lista de resultados para o produto "Cafeteira Expresso Prima Latte II, Vermelho, 110v, Oster" PO
    Quando clico no Produto PO
    Quando clico em Adicionar ao Carrinho PO
    Entao confirmo o nome do produto como "Cafeteira Expresso Prima Latte II, Vermelho, 110v, Oster" e o preco de "R$ 1199.00" PO